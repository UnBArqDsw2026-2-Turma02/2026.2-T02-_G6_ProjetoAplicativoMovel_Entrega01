# SubEquipe_03 — Relatório de Arquitetura de Software

**Escopo:** Fluxo de Infrações  
**Aplicação analisada:** aplicativo Detran-DF Digital  
**Subfluxos:** 4.1 Identificação de Condutor Infrator · 4.2 Histórico de Infrações (por Condutor/Veículo) · 4.3 Processo de Penalidade de Suspensão de CNH  
...
<!-- INSERIR IMAGEM GERADA NO bpmn.io. O diagrama Mermaid abaixo serve como referência textual -->
...
```mermaid
flowchart TD
    START(("⬤ Início:<br>Infração registrada"))

    subgraph DETRAN["🏛️ Lane: Detran-DF / Autoridade de Trânsito"]
        D1["Registrar auto<br>de infração"]
        D2{"⬦ Gateway XOR:<br>Condutor identificado<br>na autuação?"}
        D3["Notificar proprietário<br>(prazo 30 dias)"]
        D4["Analisar indicação"]
        D5{"⬦ Gateway XOR:<br>Indicação válida?"}
        D6["Aplicar regra legal<br>de responsabilização"]
        D7["Associar condutor<br>à infração"]
        D14["Instaurar processo<br>de suspensão"]
        D15["Notificar condutor"]
        D17["Analisar defesa<br>/ processo"]
        D18{"⬦ Gateway XOR:<br>Defesa acolhida?"}
        D19["Arquivar processo"]
        D20["Aplicar penalidade"]
        D21["Notificar penalidade"]
        D24["Acompanhar<br>cumprimento"]
    end

    subgraph CONDUTOR["🧑 Lane: Condutor / Proprietário"]
        C1["Receber notificação"]
        C2["Indicar condutor real<br>(CPF, CNH)"]
        C10["Receber notificação<br>de instauração"]
        C11{"⬦ Gateway XOR:<br>Apresenta defesa?"}
        C12["Apresentar defesa"]
        C13["Receber decisão"]
        C14{"⬦ Gateway XOR:<br>Apresenta recurso?"}
        C15["Apresentar recurso<br>à JARI"]
        C16["Cumprir suspensão"]
        C17["Realizar curso<br>de reciclagem"]
    end

    subgraph SISTEMAS["🖥️ Lane: Sistemas (RENAINF / RENACH)"]
        S1["Atualizar histórico<br>de infrações"]
        S2["Consolidar pontuação<br>via RENAINF"]
        S3{"⬦ Gateway XOR:<br>Limite de suspensão<br>atingido?"}
        S4["Registrar penalidade<br>no RENACH"]
        S5["Atualizar habilitação"]
    end

    subgraph JARI["⚖️ Lane: JARI / Instância Recursal"]
        J1["Analisar recurso"]
        J2{"⬦ Gateway XOR:<br>Penalidade mantida?"}
        J3["Penalidade alterada"]
    end

    FIMOK(("◉ Fim:<br>Habilitação<br>regularizada"))
    FIMARQ(("◉ Fim:<br>Processo arquivado<br>ou sem suspensão"))
...
|---|---|---|
| Felipe Henrique Oliveira Sousa | Estruturação do relatório, pesquisa normativa (CTB, CONTRAN), Mapa Mental (estrutura base + subfluxo 4.1), ramo de Integridade no SIG, Engenharia Reversa do subfluxo 4.1 e modelagem BPMN do fluxo integrado | 27/08/2026 |
