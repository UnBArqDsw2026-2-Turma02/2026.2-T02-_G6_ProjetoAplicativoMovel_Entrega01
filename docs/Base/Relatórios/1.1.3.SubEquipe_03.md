...
O modelo representa o fluxo de Alteração de Categoria, escolhido por ser o subfluxo com maior número de decisões observáveis dentro do escopo delimitado.

O diagrama usa **duas raias**, separando o que é ação do condutor do que é processamento do sistema. O **gateway exclusivo** representa a fronteira descoberta na observação: categorias C, D e E encerram o fluxo no aplicativo com orientação para o atendimento presencial, enquanto as demais seguem para confirmação.

A tarefa "Registrar solicitação" está rotulada como não observada. Não concluímos a solicitação, então não temos evidência do que o sistema faz em seguida. Preferimos marcar a lacuna a preenchê-la por suposição — um modelo de Engenharia Reversa deve representar o que foi observado, não o que se imagina.

O arquivo-fonte no formato BPMN 2.0 (`.bpmn`) está versionado junto com a imagem, o que permite reabrir e editar o modelo em qualquer editor compatível com o padrão da OMG.
...
|---|---|---|
| Artur Fernandes | Compreendi a diferença entre Engenharia Progressiva e Reversa, e principalmente a distinção entre Engenharia Reversa e Reengenharia — a primeira só entende o sistema, a segunda reconstrói. Aprendi a classificar um esforço de Engenharia Reversa pelos quatro elementos (nível de abstração, completude, interatividade e direcionalidade) e apliquei isso ao nosso próprio trabalho. No NFR Framework, entendi por que um requisito não funcional precisa ser operacionalizado até virar algo mensurável: "ser seguro" não permite verificação, "exigir chave de acesso para remoção" permite. Conduzi a observação dos subfluxos, elaborei o modelo BPMN e o ramo de Perceptibilidade do SIG. | A IA generativa me ajudou a organizar os conceitos teóricos e a gerar a estrutura inicial dos arquivos de modelagem a partir dos registros de observação que produzi. O ponto crítico é que ela não podia ter feito a parte principal: sem percorrer o aplicativo, nenhuma ferramenta teria como saber que "Serviços para CNH" não contém emissão nem segunda via, ou que as categorias são filtradas pela habilitação atual. Aliás, foi justamente a suposição inicial — construída sem observação — que se mostrou errada. Isso deixou claro que a IA acelera a formalização de um modelo, mas o conteúdo do modelo tem que vir da observação. Revisei cada elemento gerado contra minhas anotações antes de aceitar. |
...
|---|---|---|
| Artur Fernandes | 1. Estrutura do relatório; 2. Observação e registro dos subfluxos; 3. Modelo BPMN do fluxo de Alteração de Categoria; 4. Rich Picture (base, atores e tensões); 5. SIG de Acessibilidade (raiz, decomposição e ramo de Perceptibilidade); 6. Seção "Processo de Engenharia Reversa Aplicado"; 7. Ponto de vista sobre uso de IA Generativa | 27/08/2026 |
