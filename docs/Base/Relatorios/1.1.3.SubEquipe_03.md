...
| Incerteza sobre ser avisado quando a CNH se tornar definitiva | Observação direta: durante o acompanhamento do fluxo da CNH, não foi identificada indicação clara sobre o recebimento de uma notificação quando a CNH se tornar definitiva |
| Receio de perder o processo da CNH por cometer uma infração | Observação indireta: durante a análise das condições para obtenção da CNH definitiva, foi identificada a possibilidade de cassação da PPD em caso de infração grave, gravíssima ou reincidência |
| Necessidade de emitir um Vale Carta quando não estiver com acesso à CNH | Observação direta: durante a investigação dos serviços relacionados à CNH, não foi identificado no aplicativo um fluxo específico para emissão ou acesso ao Vale Carta |
...
| Tensões dos subfluxos 3.3 e 3.6 | Beatriz Santos |
| Tensões dos subfluxos 3.4 e 3.5 (Consulta de CNH e 2ª Via) | Lara Souza Mota |

### NFR Framework

**SIG de Acessibilidade na notação do NFR Framework**

![SIG de Acessibilidade](../assets/sig-acessibilidade-sub03.png ':size=100%')
...
| Compreensibilidade | Utilizar linguagem simples em 100% dos textos · Exibir tooltip explicativo para 100% dos termos técnicos · Exibir indicador "passo X de N" em todas as telas do fluxo da tarefa | Beatriz Santos |
| Operabilidade | Exigir chave de acesso própria para remoção do documento · Restringir a remoção do documento apenas mediante autenticação | Lara Souza Mota |
...
| Exibir tooltip explicativo para 100% dos termos técnicos | `−` | Custo de desenvolvimento | Aumenta o custo de manutenção, pois exige revisão dos termos técnicos |
| Exibir indicador “passo X de N” em todas as telas do fluxo da tarefa | `−` | Operabilidade | Pode aumentar a quantidade de informações apresentadas simultaneamente na interface |
...
Cada integrante percorreu os subfluxos atribuídos, registrando telas, mensagens, decisões, condições de acesso e situações em que o aplicativo não permitia avançar. Os registros foram compartilhados entre os integrantes para comparar os comportamentos observados e consolidar as regras de negócio.

A execução das atividades do Foco_02 baseou-se no método de Engenharia Reversa sobre a aplicação móvel da CNH do Brasil. Cada integrante ficou responsável por percorrer de forma prática e independente os subfluxos definidos, utilizando o aplicativo real no dispositivo móvel para mapear e documentar o comportamento do sistema.

Cada subfluxo foi analisado sob três cenários distintos:
1. **Fluxo Principal (Caminho Feliz):** Execução do procedimento padrão até o encerramento com sucesso;
2. **Caminho de Exceção/Tratamento de Erro:** Provocação voluntária de falhas (como omissão de campos obrigatórios ou submissão de dados inválidos) para avaliar o comportamento do aplicativo e capturar as mensagens de validação;
3. **Interrupção de Sessão:** Abandono voluntário no meio da operação para observar a retenção do progresso e o estado do sistema.

A documentação das telas e regras de negócio seguiu rigorosamente os critérios de privacidade adotados pela subequipe, registrando os fluxos exclusivamente em texto estruturado sem o armazenamento de dados pessoais identificáveis (como CPF, foto ou endereço). Os registros observados foram compartilhados entre a equipe através de repositório Git e mensagens assíncronas no Discord/WhatsApp, onde definimos a divisão dos artefatos (Rich Picture, SIG e BPMN) e revisamos coletivamente os diagramas criados no diagrams.net (draw.io) e bpmn.io.
...
**Subfluxos observados — Lara Souza Mota**

**Subfluxos analisados:** 3.4 (Consultar CNH - Pontos, restrições, bloqueios) e 3.5 (2ª via de CNH).

**1. Divergência de escopo e redirecionamento para atendimento presencial:** 
Ao mapear o subfluxo de solicitação da 2ª via da CNH, observou-se que o aplicativo não conclui a emissão diretamente na plataforma digital para determinados perfis ou restrições de cadastro. Nesses casos, a aplicação interrompe a solicitação e orienta o condutor a realizar o agendamento para atendimento presencial no DETRAN da sua UF.

**2. Integração com bases nacionais e taxas:**
A consulta de pontuação e restrições (Subfluxo 3.4) exige comunicação em tempo real com a base de dados do DETRAN nacional. Para o subfluxo de 2ª via (Subfluxo 3.5), o aplicativo exige a confirmação do pagamento da taxa correspondente antes de disponibilizar o documento digital atualizado.

**3. Tratamento de exceções e ausência de conectividade:**
Na ausência de sinal de internet, a consulta de infrações e restrições atualizadas fica indisponível, embora o documento baixado anteriormente continue visível localmente na carteira digital.

**Subfluxos 3.3 e 3.6 — Beatriz Santos**

**1. CNH Definitiva.** O fluxo parte de uma PPD ativa e acompanha o período de 12 meses, enquanto o sistema monitora o histórico de infrações. Ao final do período, o sistema verifica se há infração grave, gravíssima ou reincidência. Caso exista, a PPD é cassada, o condutor é notificado e o processo de 1ª habilitação deve ser reiniciado. Caso não exista, o processo é concluído e o status da CNH é atualizado automaticamente para definitiva. O modelo também prevê a disponibilização de versão impressa, opcional, e a solicitação de 2ª via mediante taxa.

**2. Vale Carta.** O levantamento inicial considerava o Vale Carta como um possível subfluxo relacionado à CNH. A pesquisa realizada durante a Engenharia Reversa indicou que o Vale Carta é um documento provisório destinado a situações em que o condutor não está de posse da carteira, como nos casos em que ainda aguarda a entrega do documento. Entretanto, ao percorrer o aplicativo CNH do Brasil, não foi encontrado um fluxo específico para emissão, consulta ou gerenciamento de Vale Carta. Dessa forma, o resultado da Engenharia Reversa foi a identificação de uma ausência de funcionalidade: o serviço previsto no levantamento não corresponde a uma funcionalidade disponível no aplicativo analisado.
...
Escolhemos essa decomposição porque desenhar todos os subfluxos expandidos num único diagrama deixaria o modelo ilegível. Com subprocessos, dá para ver o encadeamento geral numa tela só e detalhar cada parte separadamente.
...
| 3.4 Consultar CNH e 3.5 2º via da CNH | `assets/bpmn-sub03-3-4-3-5.png` | Lara Mota |
| 3.6 CNH Definitiva | `assets/bpmn-sub03-3-6.svg` | Beatriz Santos |

#### Subfluxo 3.4 e 3.5 - Consulta de CNH e 2ª Via

O diagrama abaixo representa os subfluxos de consulta de pontuação/restrições e a solicitação de 2ª via da CNH.

![Subfluxo 3.4 e 3.5 - Consulta de CNH e 2ª Via](../assets/bpmn-sub03-3-4-3-5.png)


#### Subfluxo 3.6 - CNH Definitiva

O diagrama abaixo representa o subfluxo para aquirir a CNH definitiva para quem tem permissão.

![Subfluxo 3.6 - CNH Definitiva](../assets/bpmn-sub03-3-6.svg)

#### Recursos de notação que usamos

| Recurso | Onde usamos | Por quê |
|---|---|---|
| Pool e raias (lanes) | Todos | Separar o que é ação do condutor do que é processamento do sistema |
| Evento de início | Todos | Indicar o ponto em que cada processo é iniciado, como o acesso aos serviços ou a seleção/consulta da CNH. |
| Eventos de fim | Todos | Representar os diferentes pontos de encerramento dos processos, de acordo com o resultado de cada caminho. |
| Gateway exclusivo | Todos | Representar decisões em que apenas um dos caminhos possíveis pode ser seguido. |
| Múltiplos eventos de fim | Todos | Diferenciar os possíveis desfechos do processo, como encaminhamento ao Detran, encerramento por restrição, geração da 2ª via, cassação da PPD ou conclusão do processo. |
...
|---|---|---|
| Artur Fernandes | Entendi a diferença entre Engenharia Progressiva e Reversa, e principalmente que Engenharia Reversa não é a mesma coisa que Reengenharia — uma só entende e documenta o sistema, a outra usa esse entendimento pra reconstruir. Também aprendi a classificar o esforço pelos quatro elementos (nível de abstração, completude, interatividade e direcionalidade), e a gente aplicou isso ao nosso próprio trabalho no relatório. No NFR Framework, o que mais ficou claro pra mim foi por que a operacionalização tem que ser mensurável: não dá pra verificar "ser seguro", mas dá pra verificar "exigir chave de acesso para remover o documento". Fiz a observação dos fluxos de Alteração de Categoria e da gestão do documento digital, montei o modelo BPMN e o ramo de Perceptibilidade do SIG. | Usei IA pra organizar a parte teórica e pra gerar a estrutura inicial dos arquivos de modelagem a partir das minhas anotações. O que ficou evidente é que ela não tinha como fazer a parte principal: sem abrir o aplicativo e percorrer as telas, nenhuma ferramenta saberia que as opções de categoria são filtradas pela habilitação atual, que C, D e E são encaminhadas ao Detran, ou que a remoção do documento pede uma senha diferente do login. Tanto que a nossa lista inicial de subfluxos estava errada, e só a observação mostrou isso. Revisei tudo que foi gerado contra as minhas anotações antes de aceitar, e onde eu não observei o que acontecia deixei marcado como "não observado" no diagrama em vez de completar por suposição. |
| Lara Souza Mota | Compreendi a modelagem de subprocessos expandidos (BPMN 2.0 )focando nas regras de negócio para consulta e solicitação de 2ª ,alem além do gerenciamento de documentação do MkDocs | Auxiliou na geração de comandos CLI para conversão de vetores SVG em PNG  e no suporte à organização e validação do relatório no MkDocs. |
| Beatriz Santos | Aprendi a diferenciar uma funcionalidade realmente existente de uma hipótese levantada antes da observação. A análise do Vale Carta mostrou a importância de validar o escopo por meio da Engenharia Reversa. | Foi utilizada como apoio na organização e revisão de documentação, além de esclarecer aspectos dos artefatos. Usei principalmente as ferramentas Claude e ChatGPT|
...
|---|---|---|
| Artur Fernandes | Observação e registro dos subfluxos relacionados à Alteração de Categoria e gestão do documento digital; identificação e documentação das regras de negócio e limitações observadas na Engenharia Reversa; elaboração do BPMN do subfluxo de Emissão de CNH; contribuição para a estrutura do Rich Picture, incluindo atores e tensões dos subfluxos observados; elaboração do ramo de Perceptibilidade no SIG de Acessibilidade; organização, revisão e atualização das seções correspondentes no relatório. | 27/08/2026 |
| Lara Souza Mota | Observação e registro dos subfluxos 3.4 (Consulta CNH) e 3.5 (2ª Via); detalhamento no Rich Picture; modelagem dos subprocessos no BPMN 2.0; atualização e ajuste do SIG de Acessibilidade (PNG) no relatório. | 27/08/2026 |
| Beatriz Santos | Levantamento e documentação dos subfluxos 3.3 (Vale Carta) e 3.6 (CNH Definitiva); identificação da ausência do fluxo Vale Carta no aplicativo; elaboração do BPMN do subfluxo 3.6; contribuição para o Rich Picture e SIG de Acessibilidade; revisão e atualização das seções correspondentes no relatório. | 27/08/2026 |
