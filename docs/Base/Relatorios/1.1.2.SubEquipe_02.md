...
Além da vizualização no próprio aplicativo foi acessado o [site do senatran](https://portalservicos.senatran.serpro.gov.br/static/carteiradigital/tutoriais/html/demo_22.html) para ver funções que necessitam de infrações para prosseguir (usado para se aprofundar no fluxo de infrações por veículo)
...
| 4.2 Visualização de Infrações | `Vizualização de Infrações` | Matheus Pinheiro (infração por veículo: visualização/solicitação de boleto e solicitação de descontos) |
